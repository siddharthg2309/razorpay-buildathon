...
import { page, pageHead } from "./render.js";
  ...
  void (async () => {
    ...
    try {
      ...
      if (path === "/") html = await batchScreen();
        ...
        res.end(page("not found", "", pageHead("Not found", "No such screen.")));
        return;
      }
      ...
      res.end(page("error", "", pageHead("Something broke", "The stack is below, rather than a blank page.") + `<pre>${String((err as Error).stack ?? err)}</pre>`));
    }
  })();
});
